        ...
        def eval(_valload, saving, device):
          ...
          for step, batch in enumerate(_valload):
            ...
            with torch.no_grad():
              ...
              # Convert logits to probabilities and gather all predictions and labels
          ...
          # Compute metrics
          preds = torch.FloatTensor(all_preds)
          labels = torch.IntTensor(all_labels)
          ...
          auroc_val = auroc(preds, labels).item()
          ap_val = ap(preds, labels).item()
          pres_val = pres(preds, labels).item()
          recall_val = recall(preds, labels).item()

          print(f"AUROC: {auroc_val}, Average Precision: {ap_val}, Precision: {pres_val}, Recall: {recall_val}")
