...
class train_behrt():
    def __init__(self, src, age, sex, ethni, ins, target_data):
        ...
        if not os.path.exists(file_config['model_path']):
            os.makedirs(file_config['model_path'])
        ...
        test_loss, test_cost, test_pred, test_label = eval(testload, True, train_params['device'])
        ...
        print(f"AUROC: {auroc(preds, labels)}")
        print(f"Average Precision: {ap(preds, labels)}")
        print(f"Precision: {precision(preds, labels)}")
        print(f"Recall: {recall(preds, labels)}")
...
def run_epoch(e, trainload, device):
    ...
    for step, batch in enumerate(trainload):
        ...
        logits = behrt(input_ids, age_ids, gender_ids, ethni_ids, ins_ids, segment_ids, posi_ids, attention_mask=attMask)
